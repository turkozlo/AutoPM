...
def test_session_restart():
    ...
    with patch('pm_agent.main.LLMClient') as MockLLM:
        ...
        mock_instance.judge_step.return_value = {"passed": True, "critique": "OK", "score": 5}
        ...
        with patch('pm_agent.main.safe_input', return_value='exit'):
            ...
            with patch('pm_agent.main.DataProfilingAgent') as MockAgent:
                ...
                instance.run.return_value = '{"process_mining_readiness": {"level": "High"}}'
        ...
        assert mock_instance.judge_cumulative_progress.call_count >= 1, "Cumulative judge should have been called"
        ...
        if mock_instance.judge_cumulative_progress.call_count > 1:
            print("Verification: Session restart logic triggered correctly (judge called multiple times).")
        else:
