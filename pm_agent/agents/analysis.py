...
class ProcessAnalysisAgent:
    ...
    def run(
        self, pm_columns: Dict[str, str], output_dir: str = ".", feedback: str = ""
    ) -> str:
        """
        ...
        if timestamp_col in df.columns:
            try:
                ...
                df[timestamp_col] = [
                    ...
                    for t in df[timestamp_col]
                ]
            except Exception as e:
                col_type = str(type(df[timestamp_col]))
                return json.dumps(
                    {
                        "error": f"Failed to convert timestamp column '{timestamp_col}' (Type: {col_type}) to datetime: {e}"
                    },
                    ensure_ascii=False,
                )
        ...
        cols_to_drop = [
            c
            for c in pm4py_cols
            if c in df.columns and c not in [case_col, activity_col, timestamp_col]
        ]
        if cols_to_drop:
        ...
        try:
            ...
            return json.dumps(
                {
                    "error": f"pm4py formatting failed: {e}. Columns: {list(df.columns)}, Activity: {activity_col}"
                },
                ensure_ascii=False,
            )
        ...
        try:
            ...
            if case_durations_sec is not None and len(case_durations_sec) > 0:
                ...
                val_median, unit_median = self.get_best_unit(
                    np.median(case_durations_sec)
                )
                val_p95, unit_p95 = self.get_best_unit(
                    np.percentile(case_durations_sec, 95)
                )
            ...
            sorted_df["duration_to_next"] = (
                sorted_df.groupby(case_col_std)[timestamp_col_std]
                .diff()
                .shift(-1)
                .dt.total_seconds()
            )
            ...
            if "duration_to_next" in sorted_df.columns:
                b_stats = (
                    sorted_df.groupby(activity_col_std)["duration_to_next"]
                    .mean()
            ...
            try:
                ...
                pm4py.save_vis_performance_dfg(
                    perf_dfg, start_acts, end_acts, perf_dfg_path
                )
                rel_perf_dfg_path = os.path.basename(perf_dfg_path)
            except Exception as vis_e:
                print(
                    f"Warning: Performance DFG visualization failed (likely missing Graphviz): {vis_e}"
                )
                ...
                try:
                    ...
                    if bottlenecks:
                        ...
                        fallback_path = os.path.join(
                            output_dir, "process_performance_bottlenecks.png"
                        )
            ...
            loops = [
                {"activity": k[0], "count": int(v)}
                for k, v in dfg_basic.items()
                if k[0] == k[1]
            ]
            ...
            if case_durations_sec is not None and len(case_durations_sec) > 0:
                ...
                for cid, dur in long_cases.items():
                    val, unit = self.get_best_unit(dur)
                    anomalies.append(
                        {"case_id": str(cid), "duration": round(val, 2), "unit": unit}
                    )

            # Detailed Performance Summary for the Judge
            time_range_days = (
                formatted_df[timestamp_col_std].max()
                - formatted_df[timestamp_col_std].min()
            ).days
            mean_str = (
                f"{duration_stats['mean']['value']} {duration_stats['mean']['unit']}"
                if duration_stats
                else "N/A"
            )
            p95_str = (
                f"{duration_stats['p95']['value']} {duration_stats['p95']['unit']}"
                if duration_stats
                else "N/A"
            )
            median_str = (
                f"{duration_stats['median']['value']} {duration_stats['median']['unit']}"
                if duration_stats
                else "N/A"
            )
            ...
            if bottlenecks:
                b_list = [
                    f"{b['activity']} ({b['mean_duration']} {b['unit']})"
            ...
            perf_summary = (
                f"АНАЛИЗ ПРОИЗВОДИТЕЛЬНОСТИ: Среднее время выполнения кейса составляет {mean_str}, "
                f"медиана {median_str}, при этом 95% кейсов завершаются в пределах {p95_str}. "
                ...
                f"ДОКАЗАТЕЛЬСТВА: Расчеты выполнены через pm4py.get_all_case_durations(). "
                f"{f'Визуализация производительности сохранена в [process_performance_dfg.png]({rel_perf_dfg_path}).' if rel_perf_dfg_path else ''}"
            )
            ...
            return json.dumps(
                {"error": f"Process analysis failed: {e}"}, ensure_ascii=False
            )
