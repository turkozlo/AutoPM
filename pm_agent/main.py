            ...
            while step_count < MAX_STEPS:
                ...
                try:
                    if tool_name == "Data Profiling":
                        ...
                        current_step_result_str = f"Data Profiling completed. Readiness: {artifacts['profiling'].get('process_mining_readiness', {}).get('level')}"
            ...
            try:
                ...
                if tool_call and current_df is not None:
                    ...
                    if "error" in tool_result:
                        ...
                        print(f"🔄 Переключаюсь на Code Interpreter...")
                        needs_code = True
                    elif needs_code:
                        ...
                        if is_valid is False:
                            ...
                            print(f"🔄 Результат инструмента не устраивает. Переключаюсь на Code Interpreter для точного решения...")
                            needs_code = True
                ...
                if (needs_code or answer_text is None) and current_df is not None:
                    ...
                    calc_keywords = ["сколько", "какой процент", "посчитай", "вычисли", "найди", "покажи", "подсчитай", "среднее", "медиана", "топ", "редкий", "частый"]
                        ...
                        for attempt in range(MAX_CODE_ATTEMPTS):
                            ...
                            if exec_result["success"]:
                                ...
                                if verification.get("is_valid", True) is False:
                                    ...
                                    previous_error = f"Результат был получен, но он некорректен: {verification.get('critique')}. {verification.get('suggestion')}"
