...
def get_value_counts(
    df: pd.DataFrame, column: str, top_n: int = 10, normalize: bool = False
) -> Dict[str, Any]:
    ...
    return {
        ...
        "values": [
            {"value": str(idx), "count": float(v) if normalize else int(v)}
...
def filter_contains(
    df: pd.DataFrame, column: str, substring: str, case_sensitive: bool = False
) -> Dict[str, Any]:
    ...
    filtered = df[
        df[column].astype(str).str.contains(substring, case=case_sensitive, na=False)
    ]
    return {
        "original_rows": len(df),
        "filtered_rows": len(filtered),
...
def group_and_count(df: pd.DataFrame, group_by: str, top_n: int = 10) -> Dict[str, Any]:
    ...
    return {
        ...
        "top_groups": [
            {"group": str(idx), "count": int(v)} for idx, v in grouped.items()
        ],
    }
...
def group_and_aggregate(
    ...
    return {
        ...
        "agg_func": agg_func,
        "results": [
            {"group": str(idx), "value": round(float(v), 4) if pd.notna(v) else None}
...
def get_quantiles(
    df: pd.DataFrame, column: str, q: List[float] = [0.25, 0.5, 0.75]
) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "column": column,
            "quantiles": {
                f"{int(k * 100)}%": round(float(v), 4) for k, v in quantiles.items()
            },
        }
    except Exception as e:
        return {"error": str(e)}
...
def calculate_path_frequency(
    df: pd.DataFrame, case_col: str, activity_col: str, top_n: int = 5
) -> Dict[str, Any]:
    """Calculates the frequency of unique process paths (traces)."""
    try:
        if "timestamp" in df.columns:
            traces = (
                df.sort_values(by=[case_col, "timestamp"])
...
def get_rarest_paths(
    df: pd.DataFrame, case_col: str, activity_col: str, bottom_n: int = 5
) -> Dict[str, Any]:
...
def get_case_duration_stats(
    df: pd.DataFrame, case_col: str, timestamp_col: str
) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "mean_duration_hours": round(
                case_durations["duration_seconds"].mean() / 3600, 2
            ),
            "median_duration_hours": round(
                case_durations["duration_seconds"].median() / 3600, 2
            ),
            "min_duration_hours": round(
                case_durations["duration_seconds"].min() / 3600, 2
            ),
            "max_duration_hours": round(
                case_durations["duration_seconds"].max() / 3600, 2
            ),
            "std_duration_hours": round(
                case_durations["duration_seconds"].std() / 3600, 2
            ),
        }
    except Exception as e:
        return {"error": str(e)}
...
def get_activity_frequency(
    df: pd.DataFrame, activity_col: str, top_n: int = 10
) -> Dict[str, Any]:
...
def count_cases_and_activities(
    df: pd.DataFrame, case_col: str, activity_col: str
) -> Dict[str, Any]:
...
def find_bottlenecks(
    ...
    try:
        ...
        df_temp["wait_time"] = (
            df_temp[timestamp_col] - df_temp["prev_timestamp"]
        ).dt.total_seconds() / 3600  # hours

        bottlenecks = (
            df_temp.groupby(activity_col)["wait_time"]
            .mean()
            .sort_values(ascending=False)
            .head(top_n)
        )
        ...
        return {
            "bottlenecks": [
                ...
                for act, wait in bottlenecks.items()
                if pd.notna(wait)
            ]
        }
    except Exception as e:
...
def execute_tool(
    tool_name: str, args: Dict[str, Any], df: pd.DataFrame
) -> Dict[str, Any]:
    """Executes a tool by name with given arguments."""
    if tool_name not in CHAT_TOOLS:
        return {
            "error": f"Неизвестный инструмент: {tool_name}. Попробуй один из: {', '.join(CHAT_TOOLS.keys())}"
        }
